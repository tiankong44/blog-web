...
<template>
 <body background="static\images\email-pattern.png">
  <div class="">
    ...
    <br />
    <el-row :gutter="30">
      <el-col :span="4"><div class="hide">111</div></el-col>
      <el-col :span="16">
        <!-- <img class="layer-photos-demo parentIframe" src="http://images.tiankong44.vip/1582887423346" alt="图片名" /> -->
        <el-timeline>
          <el-timeline-item :timestamp="date.uploadDate" placement="top" v-for="date in album" :key="date.uploadDate">
            <el-card>
              <span v-for="item in date.albumList" :key="item.id">
                <!-- <el-popover trigger="click"> -->
                <span>
                  <img v-image-preview :src="item.path" slot="reference" width="260px" height="200px" @click="open(item.path)" :alt="item.imgName" />
                  &nbsp;
                </span>
                <!-- </el-popover> -->
              </span>
...
export default {
  ...
  methods: {
    getAlbum() {
      ...
      this.request
        .postJson(this.blogapi.getAlbum, param)
        .then((res) => {
          if (res.code == 0) {
            this.album = res.data.list
